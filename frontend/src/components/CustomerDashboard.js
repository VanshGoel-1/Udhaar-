        ...
        return (
            <div>
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={onLogout} className="bg-red-500 text-white py-1 px-3 rounded-lg hover:bg-red-600 text-sm">Logout</button>
                </div>
                            ...
                            {isLoading ? <p className="p-4">Loading...</p> : products.map(product => {
                                ...
                                return (
                                    <li key={product.id} className="p-4 flex justify-between items-center list-none border-b">
                                        <div><p className="font-semibold">{product.name}</p><p className="text-sm text-gray-500">₹{product.price.toFixed(2)}</p></div>
                                        {quantityInCart === 0 ? (
                                            ...
                                            <div className="flex items-center gap-2"><button onClick={() => updateCartQuantity(product, quantityInCart - 1)} className="bg-gray-200 w-8 h-8 rounded-full">-</button><span className="font-bold w-8 text-center">{quantityInCart}</span><button onClick={() => updateCartQuantity(product, quantityInCart + 1)} className="bg-gray-200 w-8 h-8 rounded-full">+</button></div>
                                        )}
                                    </li>
                                );
                            })}
                    ...
                    <div className="md:col-span-1">
                        <div className="bg-gray-50 p-4 rounded-lg border sticky top-8">
                            ...
                            {cart.length === 0 ? <p>Your cart is empty.</p> : (
                                <>
                                    <ul className="mb-4 max-h-60 overflow-y-auto">{cart.map(item => (<li key={item.id} className="py-2 flex justify-between"><span>{item.name} x {item.quantity}</span><span>₹{(item.price * item.quantity).toFixed(2)}</span></li>))}</ul>
                                    <div className="border-t pt-4"><div className="flex justify-between font-bold text-xl"><span>Total</span><span>₹{cartTotal.toFixed(2)}</span></div><button onClick={placeOrder} disabled={isLoading} className="w-full mt-4 bg-green-600 text-white py-2 rounded-lg hover:bg-green-700 disabled:bg-green-300">{isLoading ? 'Placing...' : 'Place Order'}</button></div>
                                </>
                            )}
                        </div>
                    </div>
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <div><h2 className="text-2xl font-bold">Customer Dashboard</h2><p className="text-gray-600">Welcome, {user.name}! Choose a shop to start.</p></div>
                <button onClick={onLogout} className="bg-red-500 text-white py-2 px-4 rounded-lg hover:bg-red-600">Logout</button>
            </div>
